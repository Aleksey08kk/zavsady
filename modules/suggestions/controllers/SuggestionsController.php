<?php

namespace app\modules\suggestions\controllers;

use app\models\Suggestions;
use app\models\SuggestionsSearch;
use app\models\User;
use Yii;
use yii\web\Controller;
use yii\web\NotFoundHttpException;
use yii\filters\VerbFilter;
use yii\web\Response;

/**
 * SuggestionsController implements the CRUD actions for Suggestions model.
 */
class SuggestionsController extends Controller
{
    /**
     * @inheritDoc
     */
    public function behaviors()
    {
        return array_merge(
            parent::behaviors(),
            [
                'verbs' => [
                    'class' => VerbFilter::className(),
                    'actions' => [
                        'delete' => ['POST'],
                    ],
                ],
            ]
        );
    }

    /**
     * Lists all Suggestions models.
     *
     * @return string
     */
    public function actionIndex()
    {
        $suggestionsModel = Suggestions::find()->all();
        $userModel = User::find()->where(['id' => Yii::$app->user->identity->id])->one();

        return $this->render('index', [
            'suggestionsModel' => $suggestionsModel,
            'userModel' => $userModel
        ]);
    }

    public function actionSearch()
    {
        $searchModel = new SuggestionsSearch();
        $dataProvider = $searchModel->search($this->request->queryParams);

        return $this->render('index', [
            'searchModel' => $searchModel,
            'dataProvider' => $dataProvider,
        ]);
    }

    /**
     * Displays a single Suggestions model.
     * @param int $id ID
     * @return string
     * @throws NotFoundHttpException if the model cannot be found
     */
    public function actionView($id)
    {
        return $this->render('view', [
            'model' => $this->findModel($id),
        ]);
    }

    /**
     * Creates a new Suggestions model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return string|\yii\web\Response
     */
    public function actionCreate()
    {
        $model = new Suggestions();

        if ($this->request->isPost) {
            if ($model->load($this->request->post()) && $model->save()) {
                //return $this->redirect(['index', 'id' => $model->id]);
                return $this->redirect('index');
            }
        } else {
            $model->loadDefaultValues();
        }

        return $this->render('create', [
            'model' => $model,
        ]);
    }

    /**
     * Updates an existing Suggestions model.
     * If update is successful, the browser will be redirected to the 'view' page.
     * @param int $id ID
     * @return string|\yii\web\Response
     * @throws NotFoundHttpException if the model cannot be found
     */
    public function actionUpdate($id)
    {
        $model = $this->findModel($id);

        if ($this->request->isPost && $model->load($this->request->post()) && $model->save()) {
            return $this->redirect(['view', 'id' => $model->id]);
        }

        return $this->render('update', [
            'model' => $model,
        ]);
    }

    /**
     * Deletes an existing Suggestions model.
     * If deletion is successful, the browser will be redirected to the 'index' page.
     * @param int $id ID
     * @return \yii\web\Response
     * @throws NotFoundHttpException if the model cannot be found
     */
    public function actionDelete($id)
    {
        $this->findModel($id)->delete();

        return $this->redirect(['index']);
    }

    /**
     * Finds the Suggestions model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param int $id ID
     * @return Suggestions the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Suggestions::findOne(['id' => $id])) !== null) {
            return $model;
        }

        throw new NotFoundHttpException('The requested page does not exist.');
    }

    public function actionUpdateStatus($id)
    {
        $suggestion = Suggestions::findOne($id);
        if ($suggestion === null) {
            throw new NotFoundHttpException("Предложение не найдено.");
        }
        if (Yii::$app->request->isPost) {
            $suggestion->load(Yii::$app->request->post());
            if ($suggestion->save()) {
                Yii::$app->session->setFlash('success' . $id, 'Статус успешно изменен.');
                return $this->redirect('index');
            }
        }
        return $this->render('index', [
            'suggestion' => $suggestion,
        ]);
    }

    public function actionComment($id)
    {
        $suggestion = Suggestions::findOne($id);
        if ($suggestion === null) {
            throw new NotFoundHttpException("Предложение не найдено.");
        }
        if (Yii::$app->request->isPost) {
            $suggestion->load(Yii::$app->request->post());
            if ($suggestion->save()) {
                Yii::$app->session->setFlash('success' . $id, 'Статус успешно изменен.');
                return $this->redirect('index');
            }
        }
        return $this->render('index', [
            'suggestion' => $suggestion,
        ]);
    }
}


/*
public function actionUpdateStatus($id)
    {
        $model = $this->findModel($id);
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            Yii::$app->session->setFlash('success', 'Статус предложения изменен на: ' . $model->status);
            Yii::$app->session->set('lastUpdatedSuggestionId', $id); // Сохраняем ID последнего обновленного предложения
            return $this->redirect(['index']); // Перенаправляем на страницу со списком предложений
        }
        return $this->render('update', [
            'model' => $model,
        ]);
    }
        */